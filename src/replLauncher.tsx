import React from 'react';
import type { StatsStore } from './context/stats.js';
import type { Root } from './ink.js';
import type { Props as REPLProps } from './screens/REPL.js';
import type { AppState } from './state/AppStateStore.js';
import type { FpsMetrics } from './utils/fpsTracker.js';
type AppWrapperProps = {
  getFpsMetrics: () => FpsMetrics | undefined;
  stats?: StatsStore;
  initialState: AppState;
};
export async function launchRepl(root: Root, appProps: AppWrapperProps, replProps: REPLProps, renderAndRun: (root: Root, element: React.ReactNode) => Promise<void>): Promise<void> {
  process.stderr.write('[DEBUG] launchRepl: importing App\n');
  const {
    App
  } = await import('./components/App.js');
  process.stderr.write('[DEBUG] launchRepl: importing REPL\n');
  const replImportPromise = import('./screens/REPL.js');
  const replTimeout = setTimeout(() => {
    process.stderr.write('[DEBUG] launchRepl: REPL import TIMEOUT after 5s - still waiting\n');
  }, 5000);
  let replMod: any;
  try {
    replMod = await replImportPromise;
    clearTimeout(replTimeout);
    process.stderr.write('[DEBUG] launchRepl: REPL imported OK\n');
  } catch(e) {
    clearTimeout(replTimeout);
    process.stderr.write('[DEBUG] launchRepl: REPL import ERROR: ' + String(e) + '\n');
    if (e instanceof Error) process.stderr.write(e.stack + '\n');
    throw e;
  }
  const { REPL } = replMod;
  process.stderr.write('[DEBUG] launchRepl: calling renderAndRun\n');
  await renderAndRun(root, <App {...appProps}>
      <REPL {...replProps} />
    </App>);
  process.stderr.write('[DEBUG] launchRepl: renderAndRun done\n');
}
//# sourceMappingURL=data:application/json;charset=utf-8;base64,